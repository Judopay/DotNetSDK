        ...
        internal class CollectionsTestSource
        {
            public static IEnumerable ValidateFailureTestCases
            {
                get
                {
                    ...
                    }, JudoModelErrorCode.Amount_Not_Valid).SetName("ValidateCollectionMissingAmount");
                    // Change both following to expect Amount_Greater_Than_0 once 6.3 release deployed
                    yield return new TestCaseData(new CollectionModel
                    {
                        ReceiptId = 685187481842388992,
                        Amount = 0m
                    }, JudoModelErrorCode.Amount_Not_Valid).SetName("ValidateCollectionZeroAmount");
                    /*
                    yield return new TestCaseData(new CollectionModel
                    {
                        ReceiptId = 685187481842388992,
                        Amount = -1m
                    }, JudoModelErrorCode.Amount_Not_Valid).SetName("ValidateCollectionNegativeAmount");
                    */
                }
            }
        }
